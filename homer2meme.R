...
motif2meme <- function(inFile, alt.name="empty") {
  ...
  fanc.motif.name <- basename(file_path_sans_ext(inFile))
  if (n.motifs > 1) {
    for (i in 1:(n.motifs-1)) {
      ...
      motif_name <- paste(fanc.motif.name,i,sep="_")
      cat("MOTIF",motif_name,alt.name,name.string3,"\n",file=thisFile, append=TRUE)
