...
{
    ftp <- sub("/scratch/", "https://wangftp.wustl.edu/~", filename) %>% 
        sub("/bar/", "https://wangftp.wustl.edu/~", .)
    return(ftp)
}
...
target.jsongen <- function(target.out.full.paths, samples=NULL, out.json=NULL, prefix, suffix, track_type) {
  ...
  jsongen <- data.frame(name = basename(files), url = bash2ftp(files), type = track_type)
...
qatacview.jsongen <- function(df, out.json=NULL, replace=F) {
  # takes in a dataframe with 3 columns: collection; pipedir, samples. It looks something like this:
  # collection                        pipedir        samples
  # 1    NK_fanc ~/4dn/nk/fanc/sth/target_pipe/ Ly49A_neg_rep1
  # 2    NK_fanc ~/4dn/nk/fanc/sth/target_pipe/ Ly49A_neg_rep2
  # 3    NK_fanc ~/4dn/nk/fanc/sth/target_pipe/ Ly49A_neg_rep3
  # 4    NK_fanc ~/4dn/nk/fanc/sth/target_pipe/ Ly49A_pos_rep1
  # 5    NK_fanc ~/4dn/nk/fanc/sth/target_pipe/ Ly49A_pos_rep2
  # 6    NK_fanc ~/4dn/nk/fanc/sth/target_pipe/ Ly49A_pos_rep3
  if (is.character(df))
